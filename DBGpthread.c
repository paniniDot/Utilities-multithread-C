...
int DBGpthread_mutex_trylock(pthread_mutex_t *AddrMutex, const char *stringMsg)
{
        int rc;
#ifdef DEBUG
        if(stringMsg) printf("trylock %s\n",stringMsg);
#endif
        rc = pthread_mutex_trylock(AddrMutex);
        if(rc==EBUSY) {
#ifdef DEBUG
                if(stringMsg) printf("trylock %s do not acquire lock\n",stringMsg);
#endif
                return(rc);
        }
        if(rc) PrintERROR_andExit(rc,"pthread_mutex_lock failed");
...
void DBGnanosleep( unsigned long int NumNanoSeconds, const char *stringMsg )
{
...
void DBGpthread_create( pthread_t *thread, const pthread_attr_t *attr, void *(*start_routine) (void *), void *arg, const char *stringMsg )
{
    int rc;
#ifdef DEBUG
    if(stringMsg) printf("create %s\n",stringMsg);
#endif
